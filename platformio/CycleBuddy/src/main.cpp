...
#define ROTARY_S1_PIN 3
#define ROTARY_S2_PIN 0
#define ROTARY_KEY_PIN  1
...
uint16_t  bg_color =0;
uint16_t encoderAngle = 0;
AiEsp32RotaryEncoder rotaryEncoder = AiEsp32RotaryEncoder(ROTARY_S1_PIN, ROTARY_S2_PIN, ROTARY_KEY_PIN, ROTARY_ENCODER_VCC_PIN, ROTARY_ENCODER_STEPS);
...
void setup()   {
  ...
  rotaryEncoder.setBoundaries(0,240,false);
...
void loop() {
 // uint16_t angle = random(241); // random speed in range 0 to 240
  if(rotaryEncoder.encoderChanged())  {
    encoderAngle = (uint16_t)rotaryEncoder.readEncoder();
    if( encoderAngle > 240) {
      encoderAngle = 240;
    }
    if (encoderAngle < 0) {
